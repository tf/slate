...
var MemoizedElement = React__default.memo(Element, (prev, next) => {
  return prev.decorate === next.decorate && prev.element === next.element && prev.renderElement === next.renderElement && prev.renderLeaf === next.renderLeaf && isRangeListEqual(prev.decorations, next.decorations) && (prev.selection === next.selection || !!prev.selection && !!next.selection && slate.Range.equals(prev.selection, next.selection) && prev.selectionStart === next.selectionStart && prev.selectionEnd === next.selectionEnd);
});
/**
...
var Children = props => {
  ...
  for (var i = 0; i < node.children.length; i++) {
    ...
    var selStart = !!selection && !!sel && slate.Point.equals(slate.Range.start(selection), slate.Range.start(sel));
    var selEnd = !!selection && !!sel && slate.Point.equals(slate.Range.end(selection), slate.Range.end(sel));
    var ds = decorate([n, p]);
