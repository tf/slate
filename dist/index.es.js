...
import { Path, Node as Node$1, Editor, Text as Text$1, Range, Element as Element$1, Point, Transforms } from 'slate';
...
var MemoizedElement = React.memo(Element, (prev, next) => {
  return prev.decorate === next.decorate && prev.element === next.element && prev.renderElement === next.renderElement && prev.renderLeaf === next.renderLeaf && isRangeListEqual(prev.decorations, next.decorations) && (prev.selection === next.selection || !!prev.selection && !!next.selection && Range.equals(prev.selection, next.selection) && prev.selectionStart === next.selectionStart && prev.selectionEnd === next.selectionEnd);
});
/**
...
var Children = props => {
  ...
  for (var i = 0; i < node.children.length; i++) {
    ...
    var selStart = !!selection && !!sel && Point.equals(Range.start(selection), Range.start(sel));
    var selEnd = !!selection && !!sel && Point.equals(Range.end(selection), Range.end(sel));
    var ds = decorate([n, p]);
